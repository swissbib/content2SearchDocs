...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
            ...
            <xsl:variable name="belongsToRepository">
                <xsl:choose>
                    <xsl:when test="some $x in $networks/database
                         satisfies $x = swissbib:selectNetworks(.)">
                    ...
                    <xsl:when test="some $x in $sublibs/library
                        satisfies $x = /record/datafield[@tag='956']/subfield[@code='a']">
                        <xsl:value-of select="'true'" />                    
                    </xsl:when>
                    <xsl:when test="some $x in $networks/database
                        satisfies $x = /record/datafield[@tag='956']/subfield[@code='B']">
                        <xsl:value-of select="'true'" />                    
                    </xsl:when>
                    <xsl:otherwise>
    ...
    <xsl:function name="swissbib:selectNetworks">
        <xsl:param name="structure" />
        <xsl:variable name="tsublibs">
            <xsl:for-each select="$structure/record/datafield[@tag='035']/subfield[@code='a']">
                <xsl:analyze-string select="." regex="\((.*)\)">
                    <xsl:matching-substring>
                        <xsl:value-of select="regex-group(1)"  /><xsl:value-of select="'##'" />
                    </xsl:matching-substring> 
                    <xsl:non-matching-substring />
                </xsl:analyze-string>        
            </xsl:for-each>
        </xsl:variable>
        <xsl:sequence select="fn:tokenize($tsublibs,'##')" />
    </xsl:function>
