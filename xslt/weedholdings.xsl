...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    * swissbib gruen kann und sollte die Struktur der 949 identisch uebernehmen, dazu muessen fuer eine
    ...
    <xsl:template match="datafield[@tag='852']">
        <xsl:if test="matches(subfield[@code='B'], 'IDSBB|SNL|CHARCH|RETROS') or matches(subfield[@code='b'], 'E2[45]|E3[019]|E44|E5[069]|E60|E7[15]|E96|N0[123]')">
    ...
    <xsl:template match="datafield[@tag=949]/subfield">
        <xsl:if test="matches(@code, 'b|B|j|s|z')">
            <xsl:copy-of select="." />
        </xsl:if>
    </xsl:template>
    <!-- b=location code, B=network, j=call number, s=second call number (used by NEBIS), z=Description or public note -->


    <!--
       wir nehmen 956 kopmplett GH: 20120502
