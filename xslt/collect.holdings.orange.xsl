...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    * erstellt Holdings und Items soweit relevant f. Orange
    ...
    <xsl:template match="datafield[@tag='852']">
        <xsl:if test="matches(subfield[@code='B'], 'IDSBB|SNL|RETROS') or matches(subfield[@code='F'], 'E30|E44|E75|E96|N01|N02|N03')">
            <xsl:copy-of select="."/>
            <xsl:apply-templates/>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="datafield[@tag='949']">
        <xsl:if test="matches(subfield[@code='B'], 'IDSBB|SNL|RETROS') or matches(subfield[@code='F'], 'E30|E44|E75|E96|N01|N02|N03|RETROS')">
            <xsl:copy-of select="."/>
            <xsl:apply-templates/>
        </xsl:if>
    </xsl:template>
